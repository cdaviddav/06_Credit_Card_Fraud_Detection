...
print('Frauds', round(strat_test['Class'].sum()/strat_test['Class'].count()*100,2), '% after StratifiedShuffleSplit in test set')


df_train = strat_train
...
#view_clusters(X_reduced_tsne, X_reduced_pca, X_reduced_svd)
...
param_grid_LR = [{'reduce_dim__n_components': [5, 15, 19],
               'classification__penalty': ['l1', 'l2'],
               'classification__C': [0.001, 0.01, 0.1, 1, 10, 100, 1000]
              }]

param_grid_KNC = [{'reduce_dim__n_components': [5, 15, 19],
               'classification__n_neighbors': list(range(2,5,1)),
               'classification__algorithm': ['auto', 'ball_tree', 'kd_tree', 'brute']
              }]

param_grid_SVC = [{'reduce_dim__n_components': [5, 15, 19],
               'classification__C': [0.5, 0.7, 0.9, 1],
               'classification__kernel': ['rbf', 'poly', 'sigmoid', 'linear']
              }]

param_grid_RFC = [{'reduce_dim__n_components': [5, 15, 19],
               'classification__max_depth': [4, 9, 20, 50, 100],
               'classification__verbose': [0, 1]
              }]


classifiers_name = ("LogisticRegression", "KNearest", "SupportVectorClassifier", "RandomForestClassifier")

classifiers_grid = (param_grid_LR, param_grid_KNC, param_grid_SVC, param_grid_RFC)
...
for i in range(0,4):
    ...
    pipeline = Pipeline([
        ...
        # scaling because linear models are sensitive to the scale of input features
        ('classification', classifiers[i]),
    ])

    pipe_best_params = classification_pipeline(x_train, y_train, pipeline, 5, 'accuracy', classifiers_grid[i])

    if classifiers_name[i] == "LogisticRegression":
        pipe_best = Pipeline([
            ('reduce_dim', PCA(n_components = pipe_best_params['reduce_dim__n_components'])),
            ('feature_scaling', MinMaxScaler()),
            ('classification', LogisticRegression(
                penalty = pipe_best_params['classification__penalty'],
                C = pipe_best_params['classification__C'],))
        ])
        with open(os.path.join(settings.csv, 'LogisticRegression.json'), 'w') as lr:
            json.dump(pipe_best_params, lr)
        lr.closed

    elif classifiers_name[i] == "KNearest":
        pipe_best = Pipeline([
            ('reduce_dim', PCA(n_components = pipe_best_params['reduce_dim__n_components'])),
            ('feature_scaling', MinMaxScaler()),
            ('classification', KNeighborsClassifier(
                n_neighbors = pipe_best_params['classification__n_neighbors'],
                algorithm = pipe_best_params['classification__algorithm'],))
        ])
        with open(os.path.join(settings.csv, 'KNearest.json'), 'w') as kn:
            json.dump(pipe_best_params, kn)
        kn.closed

    elif classifiers_name[i] == "SupportVectorClassifier":
        pipe_best = Pipeline([
            ('reduce_dim', PCA(n_components = pipe_best_params['reduce_dim__n_components'])),
            ('feature_scaling', MinMaxScaler()),
            ('classification', SVC(
                C = pipe_best_params['classification__C'],
                kernel = pipe_best_params['classification__kernel'],))
        ])
        with open(os.path.join(settings.csv, 'SupportVectorClassifier.json'), 'w') as sv:
            json.dump(pipe_best_params, sv)
        sv.closed

    elif classifiers_name[i] == "RandomForestClassifier":
        pipe_best = Pipeline([
            ('reduce_dim', PCA(n_components = pipe_best_params['reduce_dim__n_components'])),
            ('feature_scaling', MinMaxScaler()),
        ...
        with open(os.path.join(settings.csv, 'RandomForestClassifier.json'), 'w') as rf:
            json.dump(pipe_best_params, rf)
        rf.closed



    train_errors = evaluate_pipe_best_train(x_train, y_train, pipe_best, classifiers_name[i], binary=True)

    try:
        plot_learning_curve(pipe_best, classifiers_name[i], x_train, y_train, 'accuracy', output_file_path=settings.figures)
    except:
        pass
    ...
    evaluate_pipe_best_test(x_test, y_test, pipe_best, classifiers_name[i], binary=True)
