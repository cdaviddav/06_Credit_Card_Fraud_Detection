...
y_train_original = strat_train['Class']
x_train_original = strat_train.drop(['Class'], axis=1)
...
param_grid_KNC = {'n_neighbors': list(range(2,5,1)),
               'algorithm': ['auto', 'ball_tree', 'kd_tree', 'brute']
              }

param_grid_SVC = {'C': [0.5, 0.7, 0.9, 1],
               'kernel': ['rbf', 'poly', 'sigmoid', 'linear']
              }

param_grid_RFC = {'max_depth': [4, 9, 20, 50, 100],
               'verbose': [0, 1]
              }


classifiers_name = ("LogisticRegression", "KNearest",
...
for i in range(0,4):
    ...
    rand_log_reg = RandomizedSearchCV(classifiers[i], classifiers_grid[i], n_iter=4)

    pipeline = imbalanced_make_pipeline(SMOTE(sampling_strategy='minority'), rand_log_reg)  # SMOTE happens during Cross Validation not before..
    model = pipeline.fit(x_train, y_train)
    print('Resampled dataset shape %s' % Counter(y_train))
    pipe_best = rand_log_reg.best_estimator_
